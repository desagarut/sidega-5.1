...
<div class="content-wrapper">
	<section class="content-header">
		<h1>Lokasi: <?= $data->nama_usaha ?></h1>
		<ol class="breadcrumb">
			<li><a href="<?= site_url('hom_sid') ?>"><i class="fa fa-home"></i> Home</a></li>
			<li><a href="<?= site_url('toko_warga') ?>"> Daftar Toko Warga</a></li>
			<li class="active">Lokasi <?= $data->nama_usaha ?></li>
		</ol>
	</section>
					...
					<form id="validasi1" action="<?= $form_action ?>" method="POST" enctype="multipart/form-data" class="form-horizontal">
						...
						<div class='box-footer'>
							<div class='col-xs-12'>
								...
								<a href="<?= site_url('toko_warga') ?>" class="btn btn-social btn-flat bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Kembali"><i class="fa fa-arrow-circle-o-left"></i> Kembali</a>
...
<script>
	$(document).ready(function() {
		$('#simpan_kantor').click(function() {
			...
			$.ajax({
				type: "POST",
				url:`<?= site_url('toko_warga/lokasi_maps') ?>/${id}`,
				dataType: 'json',
				data: {
					lat: lat,
					lng: lng

...
<div class="content-wrapper">
	<section class="content-header">
		...
		<ol class="breadcrumb">
			...
			<li class="active">Data Survey Toko</li>
		</ol>
	</section>
	...
	<section class="content" id="maincontent">
		<div class="row">
			<div class="col-md-3">
				<div class="box box-info">
					<div class="box-header with-border">
						<h3 class="box-title"><strong>Form Survey</strong></h3>
					</div>
					<div class="box-body">
						<form id="validasi" action="<?= $form_action?>" method="POST" enctype="multipart/form-data">
							...
							<div class="form-group">
								...
								<select class="form-control select2" id="terdata" name="terdata" style="width: 100%;">
									...
									<?php foreach ($toko_array as $item): ?>
									<option value="<?= $item['id']?>" data-statuscovid="<?= $item['status_covid']?>" data-tgltiba="<?= $item['tanggal_awal_operasi']?>" > <?= $item['terdata_id']." - ".$item['nama']?></option>
									<?php endforeach; ?>
								</select>
							</div>
							...
							<div class="form-group">
								...
								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											<label for="tgl_jam">Tanggal Beroperasi</label>
								...
								<div class="form-group">
									<label for="suhu">No. SIUP</label>
									<input type="text" class="form-control input-sm" name="suhu" id="suhu" placeholder="Nomor Surat">
								</div>
								<div class="form-group">
									<label for="suhu">No. SITU</label>
									<input type="text" class="form-control input-sm" name="suhu" id="suhu" placeholder="Nomor Surat">
								</div>
								<div class="form-group">
									<label for="suhu">No. Akta Pendirian Usaha</label>
									<input type="text" class="form-control input-sm" name="suhu" id="suhu" placeholder="Nomor Surat">
								</div>
								...
								<div class="table-responsive-sm">
									<table class="table table-borderless table-sm">
										<thead>
											<tr>
												<th colspan="2" class="text-left">Centang jika memenuhi kriteria</th>
											</tr>
										</thead>
										<tbody>
											<tr>
												<tr>
													...
													<td>Tidak Menjual Produk Terlarang</td>
												</tr>
												<tr>
													<td width="20%" class="text-center">
														<input type="checkbox" class="form-check-input" name="sesak">
													</td>
													<td>Memiliki Karyawan</td>
												</tr>
												<tr>
													<td width="20%" class="text-center">
														<input type="checkbox" class="form-check-input" name="sesak">
													</td>
													<td>Toko Terlihat Bersih dan Rapih</td>
												</tr>
                                                <td width="20%" class="text-center">
													<input type="checkbox" class="form-check-input" name="batuk">
													</td>
													<td>Ada Lahan Parkir</td>
												</tr>
                                                <td width="20%" class="text-center">
													<input type="checkbox" class="form-check-input" name="batuk">
													</td>
												...
												<tr>
													...
													<td>Ada Tempat Cuci Tangan</td>
												</tr>
												<tr>
													<td width="20%" class="text-center">
														<input type="checkbox" class="form-check-input" name="sesak">
													</td>
													<td>Ada Tempat Duduk Bagi Pembeli</td>
												</tr>
												<tr>
													<td width="20%" class="text-center">
														<input type="checkbox" class="form-check-input" name="sesak">
							...
							<div class="form-group">
									...
									<textarea name="keterangan" class="form-control input-sm" placeholder="Keterangan Lainnya" rows="3" style="resize:none;"></textarea>
								</div>
						</form>
					</div>
			...
			<div class="col-md-9">
				<div class="box box-info">
					<div class="box-header with-border">
						<a href="<?= site_url("toko_warga/daftar/cetak/$filter_tgl/$filter_nik")?>" class="btn btn-social btn-box bg-purple btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Cetak" target="_blank"><i class="fa fa-print"></i> Cetak
						</a>
						<a href="<?= site_url("toko_warga/daftar/unduh/$filter_tgl/$filter_nik")?>" class="btn btn-social btn-box bg-navy btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block" title="Unduh" target="_blank"><i class="fa fa-download"></i> Unduh
						</a>
					</div>
					<div class="box-body">
						<div class="row">
								...
								<div class="col-sm-5">
									<div class="form-group">
										...
										<select class="form-control select2 input-sm" name="unique_nik_select" id="unique_nik_select" style="width: 100%;">
											...
											<option value="<?= $row[id_toko] ?>" > <?= $row[nik]." - ".$row[nama] ?></option>
											<?php endforeach; ?>
										</select>
									</div>
								</div>
												...
												<div class="table-responsive">
													<table class="table table-bordered dataTable table-striped table-hover">
														...
														<tbody>
															<?php
															$nomer = $paging->offset;
															foreach ($survey_toko_array as $key=>$item):
																$nomer++;
															?>
															<tr>
											...
											<div class="dataTables_paginate paging_simple_numbers">
												<ul class="pagination">
													<?php if ($paging->start_link): ?>
														<li>
															<a href="<?=site_url('toko_warga/survey/'.$paging->start_link)?>" aria-label="First"><span aria-hidden="true">Awal</span></a>
														</li>
													<?php endif; ?>

													<?php if ($paging->prev): ?>
														<li>
															<a href="<?=site_url('toko_warga/survey/'.$paging->prev)?>" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
														</li>
													<?php endif; ?>

													<?php for ($i=$paging->start_link;$i<=$paging->end_link;$i++): ?>
														<li <?=jecho($p, $i, "class='active'")?>>
															<a href="<?= site_url('toko_warga/survey/'.$i)?>"><?= $i?></a>
														</li>
													<?php endfor; ?>

													<?php if ($paging->next): ?>
														<li>
															<a href="<?=site_url('toko_warga/survey/'.$paging->next)?>" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
														</li>
													<?php endif; ?>

													<?php if ($paging->end_link): ?>
														<li>
															<a href="<?=site_url('toko_warga/survey/'.$paging->end_link)?>" aria-label="Last"><span aria-hidden="true">Akhir</span></a>
														</li>
													<?php endif; ?>
												</ul>
											</div>

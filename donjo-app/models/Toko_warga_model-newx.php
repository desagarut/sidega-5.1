<?php class Toko_warga_model extends MY_Model {
	...
	public function list_data($o=0, $offset=0, $limit=500)
	{
		switch ($o)
		{
			...
			case 3: $order_sql = ' ORDER BY status'; break;
			case 4: $order_sql = ' ORDER BY status DESC'; break;
			case 5: $order_sql = ' ORDER BY created_at'; break;
			case 6: $order_sql = ' ORDER BY created_at DESC'; break;
	...
	public function delete_toko_warga($id='', $semua=false)
	{
		if (!$semua) $this->session->success = 1;

		$this->delete($id);
		$toko_warga_produk = $this->db->select('id')->
			where('parrent', $id)->
			get('tbl_toko_warga')->result_array();
		foreach ($toko_warga_produk as $toko_warga)
		{
			$this->delete($toko_warga['id']);
		}
	}

	public function delete_all_toko_warga()
	{
		...
		foreach ($id_cb as $id)
		{
			$this->delete_toko_warga($id, $semua=true);
		}
	}
	...
	public function delete($id='', $semua=false)
	{
		...
		// oleh toko_warga lain, karena ketika mengupload
		// nama file nya belum diubah sesuai dengan
		// judul toko_warga
		$this->delete_toko_warga_image($id);
	...
	public function delete_toko_warga_image($id)
	{
	...
	public function toko_warga_lock($id='', $val=0)
	{
	...
	public function toko_warga_slider($id='', $val=0)
	{
		if ($val == 1)
		{
			// Hanya satu toko_warga yang boleh tampil di slider
	...
	public function get_toko_warga($id=0)
	{
	...
	{
		$sql = "SELECT COUNT(*) AS jml " . $this->daftar_produk_sql();
	...
	private function daftar_produk_sql()
	{
	...
	public function daftar_produk($gal=1, $o=0, $offset=0, $limit=500)
	{
		switch($o)
		{
			case 1: $order_sql = ' ORDER BY nama'; break;
		...
		$sql = "SELECT * " . $this->daftar_produk_sql();
		$sql .= $order_sql;
		$sql .= $paging_sql;
		$query = $this->db->query($sql, $gal);
		$data = $query->result_array();
	...
	//		1 - turun
	// 		2 - naik
	public function urut($id, $arah, $toko_warga='')
	{
  	$subset = !empty($toko_warga) ? array('parrent' => $toko_warga) : array('parrent' => 0);
